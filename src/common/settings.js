...
export const getActiveTabSetting = () => new Promise(resolve => {
    browser.storage.sync.get(['active'], result => resolve(!!result.active));
});

export const getSettings = () => new Promise(resolve => {
  browser.storage.sync.get(['settings'], result => resolve(result.settings));
});

export const getStoredApiOptions = async () => {
  return new Promise(resolve => {
      browser.storage.sync.get(['options'], result => resolve(result.options));
  });
};
